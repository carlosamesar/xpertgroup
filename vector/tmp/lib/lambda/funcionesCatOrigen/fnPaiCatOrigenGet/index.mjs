...
/**
 * Handler para obtener un registro de catálogo de origen por ID
...
export const handler = async (event) => {
  ...
  try {
    ...
    console.log("Buscando registro con ID:", id_origen);
    ...
    if (!result.Item) {
      console.log(`Registro con ID ${id_origen} no encontrado`);
      return createNotFoundResponse("Catálogo de origen", id_origen.toString());
    }

    // 6. Log de auditoría
    console.log(`Catálogo de origen consultado - ID: ${id_origen} por usuario: ${userPayload.sub}`);
    ...
    return createSuccessResponse(
      200, 
      responseData,
      "Catálogo de origen obtenido exitosamente"
    );
