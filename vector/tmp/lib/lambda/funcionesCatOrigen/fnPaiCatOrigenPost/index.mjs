...
/**
 * Handler para crear un nuevo registro de catálogo de origen
...
export const handler = async (event) => {
  ...
  try {
    ...
    // 7. Log de auditoría
    console.log(`Catálogo de origen creado exitosamente - ID: ${validatedData.id_origen} por usuario: ${userPayload.sub}`);

    // 8. Respuesta de éxito
    return createSuccessResponse(
      201, 
      {
        id_origen: dynamoItem.id_origen,
        descripcion: dynamoItem.descripcion,
        created_at: dynamoItem.createdAt,
        item_type: dynamoItem.item_type
      },
      "Catálogo de origen creado exitosamente"
    );
