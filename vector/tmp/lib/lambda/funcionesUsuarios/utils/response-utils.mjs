/**
 * Utilidades para generar respuestas estandarizadas para usuarios
...
export const handleDynamoDBError = (error) => {
  ...
  switch (error.name) {
    case 'ConditionalCheckFailedException':
      return createErrorResponse(409, 'El usuario ya existe o no cumple las condiciones', 'RESOURCE_CONFLICT');
    
    case 'ValidationException':
      return createErrorResponse(400, 'Datos de entrada inválidos', 'VALIDATION_ERROR');
    
    case 'ResourceNotFoundException':
      return createErrorResponse(404, 'Usuario no encontrado', 'RESOURCE_NOT_FOUND');
 ...
 */
export const createNotFoundResponse = (resourceType = 'Usuario', resourceId = '') => {
...
/**
 * Limpia datos sensibles del usuario antes de retornarlos
 * @param {Object} user - Objeto usuario de DynamoDB
 * @returns {Object} - Usuario sin datos sensibles
 */
export const sanitizeUserData = (user) => {
  if (!user) return null;

  // Crear copia del objeto sin campos sensibles
  const {
    _pk,
    _sk,
    cognito_sub,
  ...
  return {
    ...publicData,
    // Incluir solo información no sensible del código de activación si existe
    has_activation_code: !!cod_act
  };
};
